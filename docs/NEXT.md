...
- **파일럿 로스터 방문**: 체크리스트는 `docs/visit-checklist.md`에 정리(디자인 반응·실데이터 신호·이연 결정·셀프서비스 수요). gpze QR high/short 연속 4회(온도 93→90)는 **예시 레시피로 사장님이 눌러본 것으로 정리** — 기준 온도 이슈 아님. 방문 전 깨끗한 데모 QR 발급 권장. 방문 후 셀프서비스(Phase C) 착수 여부·범위 결정.
...
- **Phase C 셀프서비스 이연 결정 (2026-07-03, 방문 뒤 확정)**:
  - **컨시어지 `publish-recipe` 스크립트는 접음** — 유일 명분이 "방문 당일 현장 레시피 수정"이었는데 파일럿 레시피는 예시라 바뀔 일 없음. 발행 로직은 대시보드(Phase C)에서 RPC로 바로 구현.
  - **레시피 버전 변경 → 사슬 리셋** (실운영 필수, Phase C 발행 기능과 한 몸): 다이얼인 진행 중 로스터가 새 버전을 발행하면 옛 델타(사슬 after_snapshot)를 새 기준에 얹어 이중 보정 + 기준점(카드 병기·물 ±20% 경계·temp reset)이 흔들림. 해결 = 사슬 연결 시 "직전 조정의 recipe 버전 ≠ 현재 활성 버전이면 prev를 안 잇는다" → 깨끗한 새 카드 + "로스터리가 추천 레시피를 새로 냈어요" 고정 문구 배너(§9). 정교화: **수치 파라미터가 바뀌었을 때만 리셋**(붓기 문구·소개글만 수정은 사슬 유지) — 발행 기능 만들 때 확정. 세션이 이미 `recipe_id` 보유해 버전 비교 지점 존재. 파일럿 중엔 레시피 안 바뀌니 미구현.
  - **로그인 방식**: Supabase Auth는 방식 **공존 가능** → 지금 정하는 건 첫 수일 뿐 재작업 아님. 파일럿(로스터 1명)은 **이메일 계열로 시작**(매직링크=구현 최소지만 메일함 왕복 마찰 + Supabase 기본 메일 발송량 제한 → 실서비스 시 커스텀 SMTP 필요 / 이메일+비번=비번 관리 마찰). **카카오는 로스터 확장 온보딩 단계 투자**(개발자 앱 등록·리다이렉트 + 이메일 수집엔 비즈앱 사업자 인증 필요 → 파일럿엔 과함). 방문 질문 구체화: (1) 업무용 이메일 자주 확인하는가? (2) 관리 화면 폰 위주 vs PC 위주?
...
## 완료됨
- ✅ **디자인 1차 + 방문 체크리스트 (2026-07-03, 프로덕션 배포됨)** — 웜 뉴트럴 토큰(스톤+커피 액센트, `app/globals.css`) + Pretendard dynamic subset 셀프호스팅(기기 독립). 레시피 카드·질문·결과 3종 패널(유지/조정/한계) 재스타일, 안내 3종·랜딩·파비콘(☕). 로직·§9 문구 불변, 테스트 49 유지. 링크 카피 '(언스페셜티 나침반)'→'(언스페셜티 사이트)'. 커밋 4d6d451·a3894d4 push, 배포 확인(링크 카피+Pretendard 번들 포함). 스크린샷 갤러리 https://claude.ai/code/artifact/e3ca7ff2-05d2-4d10-91db-b994caf99c30 . 방문 체크리스트 `docs/visit-checklist.md`(e0f90cb).
- ✅ **조정 경계 가드 (2026-07-03, ADR-003, 프로덕션 배포됨)** — 온도 절대 80~100°C 클램프, 물량 스텝을 원두량 비례(standard = dose×0.5 = 비율 0.5 이동)로 변경 + 기준 물량 ±20% 경계, 분쇄도는 같은 방향 권유 연속 3회 상한(µm 추적이 없어 횟수 근사 — 다른 변수 조정은 런을 안 끊고 반대 방향만 리셋). 도달 시 챗봇 아닌 고정 문구(`LIMIT_NOTICES`, §9 잠금). 기록은 채택된 move만(무효화는 rule_source와의 차이로 사후 도출). 0004 `recent_moves_for_qr` 적용됨. 검증: 유닛 14 추가(49 pass), Playwright 실주행으로 일회용 QR에서 4회째 가드 발동·moves=[] 기록·문구 표시 확인 후 테스트 데이터 정리.
