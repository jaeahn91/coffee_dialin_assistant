...
- **[제안·미합의] 조정 경계 가드**: `applyMoves`에 온도·물량 절대 한계가 없음 — 실데이터에서 같은 QR(gpze)로 하루 만에 온도 93→90 단조 하강 확인(2026-07-03). 같은 답이 반복되면 무한정 내려감(반전 감지 감쇠는 핑퐁용이라 단조 하강엔 무력). 하한/상한 값과 도달 시 행동(§9 고정 템플릿 추가 필요)은 합의 후 구현.
